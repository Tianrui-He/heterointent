...
def test_weighted_hit_at_20() -> None:
    ...
    assert metrics["official_weighted_hit@20"] == 0.3
    assert abs(metrics["hard_ndcg@20"] - 0.875) < 1e-6
    assert 0.0 <= metrics["native_selection_score"] <= 1.0

    diagnostics = compute_ranking_metrics(df, topk=1, include_diagnostics=True)
    assert diagnostics["weighted_hit@20"] == 0.3
    assert abs(diagnostics["ndcg@20"] - 0.875) < 1e-6
...
def test_request_auc_skips_requests_without_task_positives() -> None:
    ...
    assert diagnostics["request_auc_collect"] == 1.0
    assert diagnostics["request_auc_request_rate_collect"] == 0.5
    assert pd.isna(diagnostics["request_auc_share"])
...
def test_eligible_rare_and_gap_metrics_focus_on_nontrivial_requests() -> None:
    df = pd.DataFrame(
        [
            {"request_id": 1, "item_id": 1, "score": 0.9, "click": 1, "collect": 0, "share": 0, "p_click": 0.9, "p_collect": 0.1, "p_share": 0.1},
            {"request_id": 1, "item_id": 2, "score": 0.8, "click": 0, "collect": 1, "share": 0, "p_click": 0.1, "p_collect": 0.8, "p_share": 0.1},
            {"request_id": 1, "item_id": 3, "score": 0.1, "click": 0, "collect": 0, "share": 1, "p_click": 0.1, "p_collect": 0.1, "p_share": 0.7},
            {"request_id": 2, "item_id": 4, "score": 0.7, "click": 0, "collect": 0, "share": 0, "p_click": 0.1, "p_collect": 0.1, "p_share": 0.1},
            {"request_id": 2, "item_id": 5, "score": 0.6, "click": 0, "collect": 0, "share": 0, "p_click": 0.1, "p_collect": 0.1, "p_share": 0.1},
        ]
    )
    ...
    assert metrics["candidate_count_gt_topk_rate"] == 0.5
    assert diagnostics["eligible_score"] == 0.7
    assert abs(metrics["rare_score"] - (0.4 / 0.7)) < 1e-6
    assert metrics["rare_requests"] == 1.0
    assert metrics["oracle_score_eligible"] == 1.0
    assert metrics["hard_official_capture"] == 0.7
    assert 0.0 <= metrics["native_selection_score"] <= 1.0
